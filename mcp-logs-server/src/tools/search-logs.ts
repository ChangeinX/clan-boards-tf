...
export async function searchLogsHandler(cloudWatch: CloudWatchService, args: any) {
  ...
  try {
    ...
    const content = [
      ...
      ...Array.from(logsByService.entries()).map(([service, serviceLogs]) => [
        ...
        '',
        ...serviceLogs.slice(0, 20).map(log => [ // Limit to first 20 per service for readability
