...
export async function analyzeErrorsHandler(cloudWatch, args) {
    ...
    try {
        ...
        const content = [
            ...
            analysis.errorPatterns.length > 0 ? [
                ...
                '|---------|--------|----------|------------|-----------|',
                ...analysis.errorPatterns.slice(0, 10).map(pattern => `| ${pattern.pattern} | ${pattern.count} | ${pattern.severity} | ${new Date(pattern.firstSeen).toLocaleString()} | ${new Date(pattern.lastSeen).toLocaleString()} |`)
            ...
            '| Log Stream | Entries |',
            '|------------|---------|',
            ...analysis.topLogStreams.slice(0, 10).map(stream => `| ${stream.streamName} | ${stream.count} |`),
            '',
            service ? '' : [
                '## Service Breakdown',
                '',
...
//# sourceMappingURL=analyze-errors.js.map